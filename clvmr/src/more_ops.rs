...
use crate::op_utils::{
    ...
    new_atom_and_cost, nilp, u32_from_u8,
};
#[cfg(feature = "bls-ops")]
use crate::op_utils::mod_group_order;
...
// BLS-related costs (only used when bls-ops feature is enabled)
#[cfg(feature = "bls-ops")]
...
pub fn op_sha256(a: &mut Allocator, mut input: NodePtr, max_cost: Cost) -> Response {
    ...
    while let Some((arg, rest)) = a.next(input) {
        ...
        check_cost(cost, max_cost)?;
        Digest::update(&mut hasher, blob.as_ref());
    }
    new_atom_and_cost(a, cost, hasher.finalize().as_slice())
}
...
pub fn op_pubkey_for_exp(_a: &mut Allocator, input: NodePtr, _max_cost: Cost) -> Response {
    Err(EvalErr::Unimplemented(input))
}
...
#[cfg(not(feature = "bls-ops"))]
pub fn op_point_add(_a: &mut Allocator, input: NodePtr, _max_cost: Cost) -> Response {
    Err(EvalErr::Unimplemented(input))
}
...
pub fn op_coinid(a: &mut Allocator, input: NodePtr, _max_cost: Cost) -> Response {
    ...
    let mut hasher = Sha256::new();
    Digest::update(&mut hasher, parent_coin.as_ref());
    Digest::update(&mut hasher, puzzle_hash.as_ref());
    Digest::update(&mut hasher, amount);
...
mod tests {
    ...
    fn test_sha256_atom(buf: &[u8]) {
        ...
        let mut hasher = Sha256::new();
        Digest::update(&mut hasher, [1_u8].as_slice());
        if !buf.is_empty() {
